...
public final class ComponentBuilder {
    ...
    public ComponentBuilder append(BaseComponent component, FormatRetention retention) {
        ...
        if (previous != null) {
            ...
            if (retention == FormatRetention.FORMATTING || retention == FormatRetention.ALL) {
                if (component.getColorRaw() == null) component.setColor(previous.getColorRaw());
                if (component.isBoldRaw() == null) component.setBold(previous.isBoldRaw());
                if (component.isItalicRaw() == null) component.setItalic(previous.isItalicRaw());
                if (component.isUnderlinedRaw() == null) component.setUnderlined(previous.isUnderlinedRaw());
                if (component.isStrikethroughRaw() == null) component.setStrikethrough(previous.isStrikethroughRaw());
                if (component.isObfuscatedRaw() == null) component.setObfuscated(previous.isObfuscatedRaw());

                // insertions are only in 1.9 and higher
                if(Version.getServerVersion().isNewerThan(Version.V1_8)) {
                    if (component.getInsertion() == null) component.setInsertion(previous.getInsertion());
                }

                // fonts are only in 1.16 and higher
                if(Version.getServerVersion().isNewerThan(Version.V1_15)) {
                    if (component.getFontRaw() == null) component.setFont(previous.getFontRaw());
                }
            }
        }
        parts.add(component);
    ...
    public ComponentBuilder retain(FormatRetention retention) {
        if (retention == FormatRetention.FORMATTING || retention == FormatRetention.NONE) {
            getCurrentComponent().setClickEvent(null);
            getCurrentComponent().setHoverEvent(null);
        }
        if (retention == FormatRetention.EVENTS || retention == FormatRetention.NONE) {
            getCurrentComponent().setColor(null);
            getCurrentComponent().setBold(null);
            getCurrentComponent().setItalic(null);
            getCurrentComponent().setUnderlined(null);
            getCurrentComponent().setStrikethrough(null);
            getCurrentComponent().setObfuscated(null);

            // insertions are only in 1.9 and higher
            if(Version.getServerVersion().isNewerThan(Version.V1_8)) {
                getCurrentComponent().setInsertion(null);
            }
        }
        return this;
    }
